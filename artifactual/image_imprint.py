...
def try_decode_image(image_pil: Image.Image):
	"""
	Robustly try to decode the input image.
	
	Args:
		image_pil (Image.Image): The input image to decode.
	
	Returns:
		str: The decoded string from the input image.
	"""
	decoded_info = ''
	
	# Likely a diffusion image, only the one metadata key matters
	if len(image_pil.info) == 1 and 'parameters' in image_pil.info:
		decoded_info = image_pil.info['parameters']
	
	if not decoded_info:
		for k, v in image_pil.info.items():
			if k == 'dpi': continue
			decoded_info += f'{k}: {v}\r\n'
	
	if not decoded_info:
		try:
			decoded_info = decode_imprint_data(image_pil)
		except Exception as e:
			print(f'Error decoding image: {e}')
	
	if decoded_info:
		decoded_info = decoded_info.strip()
